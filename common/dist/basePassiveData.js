...
const basePassiveData = {
    ...
    boostChassisAgility: {
        toString: (p) => `+${Math.round((p.intensity || 1) * 100)}% dodge chance`,
    },
    boostDropAmount: {
        toString: (p) => `+${Math.round((p.intensity || 1) * 100)}% drop amounts`,
    },
    boostDropRarity: {
        toString: (p) => `+${Math.round((p.intensity || 1) * 100)}% AI drop rarity`,
    },
    boostRepairSpeed: {
    ...
    boostScanRange: {
        toString: (p) => `+${Math.round((p.intensity || 1) * 100)}% scan range`,
    },
    boostSightRange: {
    ...
    boostBrake: {
        toString: (p) => `+${Math.round((p.intensity || 1) * 100)}% ship braking`,
    },
    // boostThrust: {
    //   toString: (p) =>
    ...
    extraEquipmentSlots: {
        toString: (p) => `+${Math.round(p.intensity || 1)} item slot${Math.round(p.intensity || 1) === 1 ? `` : `s`}`,
    },
    boostDamageToItemType: {
        toString: (p) => `+${Math.round((p.intensity || 1) * 100)}% damage to ${p.data?.type}s`,
    },
    scaledDamageReduction: {
        toString: (p) => `+${Math.round((p.intensity || 1) * 100)}% damage reduction`,
    },
    flatDamageReduction: {
        toString: (p) => `+${Math.round(p.intensity || 1)} flat damage reduction`,
    },
    flatSkillBoost: {
