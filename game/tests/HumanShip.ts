...
describe(`HumanShip items`, () => {
  it(`should properly update slots when upgrading chassis`, async () => {
    const data = humanShipData()
    let ship = new HumanShip(data)
    expect(ship.slots).to.equal(c.items.chassis[ship.chassis.chassisId]?.slots)
  ...
  it(`should properly clear passives on item/chassis change`, async () => {
    ...
    expect(ship.passives.length).to.be.greaterThan(basicPassiveLength)

    ship.die()
    await ship.respawn()

    expect(ship.passives.length).to.equal(basicPassiveLength)
  })
})
...
describe(`HumanShip cargo/credit distribution`, () => {
  it(`should be able to distribute credits among crew members`, () => {
    ...
    const distributeAmount = Math.floor(Math.random() * 1000)

    ship.distributeCargoAmongCrew([{ id: `credits`, amount: distributeAmount }])
    ship.crewMembers.forEach((cm) => {
      expect(cm.credits).to.be.approximately(distributeAmount / 10, 1)
    })
  })
  ...
  it(`should be able to distribute cargo among crew members`, () => {
    ...
    ship.distributeCargoAmongCrew([{ id: `carbon`, amount: distributeAmount }])
    ship.crewMembers.forEach((cm) => {
      expect(cm.heldWeight).to.be.approximately(distributeAmount / 10, 0.1)
    })
  })
  ...
  it(`should not add cargo above limit`, () => {
    ...
    ship.distributeCargoAmongCrew([{ id: `carbon`, amount: 1000 }])
    ship.crewMembers.forEach((cm) => {
  ...
  it(`should add cargo properly when there are uneven cargo amounts amongst crewMembers`, () => {
    ...
    ship.distributeCargoAmongCrew([{ id: `carbon`, amount: 10 }])
    ...
    ship.distributeCargoAmongCrew([{ id: `carbon`, amount: 19 }])
    ...
    // one person is full
    ship.distributeCargoAmongCrew([{ id: `carbon`, amount: 9 }])
    expect(ship.crewMembers[0].heldWeight).to.equal(10)
    for (let i = 1; i < 10; i++) {
    ...
    ship.distributeCargoAmongCrew([{ id: `carbon`, amount: 17 }])
    expect(ship.crewMembers[0].heldWeight).to.equal(10)
  ...
  it(`should add less cargo to a crew member who is inactive`, () => {
    ...
    ship.crewMembers[0].lastActive = Date.now() - 1000 * 60 * 60 * 24 * 365
    ship.crewMembers[1].lastActive = Date.now() - 1000 * 60 * 60 * 24
    ship.crewMembers[2].lastActive = Date.now()

    ship.distributeCargoAmongCrew([{ id: `carbon`, amount: 3 }])
    expect(ship.crewMembers[1].heldWeight).to.be.greaterThan(
      ship.crewMembers[0].heldWeight,
    )
    expect(ship.crewMembers[2].heldWeight).to.be.greaterThan(
      ship.crewMembers[1].heldWeight,
    )

    ship.distributeCargoAmongCrew([{ id: `carbon`, amount: 27 }])
    expect(ship.crewMembers[0].heldWeight).to.equal(10)
    expect(ship.crewMembers[1].heldWeight).to.equal(10)
...
describe(`HumanShip death`, () => {
  it(`should spawn a cache on death with the correct amount of currency inside`, () => {
    ...
    let crewMemberCosmetic = ship.crewMembers[0].crewCosmeticCurrency
    ...
    expect(droppedCargo.map((d) => d.id)).to.contain(`credits`)
    expect(
      droppedCargo.find((d) => d.id === `credits`)?.amount,
    ).to.be.approximately(
      crewMemberCredits * 10 * CombatShip.percentOfCurrencyKeptOnDeath +
        itemRefundValue1 * CombatShip.percentOfCurrencyKeptOnDeath,
      ship.crewMembers.length,
    )
    expect(
      droppedCargo.find((d) => d.id === `crewCosmeticCurrency`)?.amount,
    ).to.be.approximately(
      crewMemberCosmetic * 10 * CombatShip.percentOfCurrencyKeptOnDeath,
      ship.crewMembers.length,
    )
    expect(
      droppedCargo.find((d) => d.id === `shipCosmeticCurrency`)?.amount,
    ).to.equal(Math.round(2 * CombatShip.percentOfCurrencyDroppedOnDeath))
    expect(ship.shipCosmeticCurrency).to.equal(
      Math.round(2 * CombatShip.percentOfCurrencyKeptOnDeath),
    )
    ...
    expect(droppedCargo[0].amount).to.be.approximately(
      crewMemberCredits * 10 * CombatShip.percentOfCurrencyKeptOnDeath +
        itemRefundValue2 * CombatShip.percentOfCurrencyKeptOnDeath,
      ship.crewMembers.length,
    )
  })
  ...
  it(`should give the correct amount of credits back to the common fund on death`, () => {
    ...
    expect(ship.commonCredits).to.be.approximately(
      distributeAmount * CombatShip.percentOfCurrencyKeptOnDeath +
        itemRefundValue2 * CombatShip.percentOfCurrencyKeptOnDeath,
      1,
    )
  ...
  it(`should drop the correct amount of cargo on death`, () => {
    ...
    expect(droppedCargo.find((ca) => ca.id === `carbon`)?.amount).to.equal(10)
    expect(droppedCargo.find((ca) => ca.id === `oxygen`)?.amount).to.equal(12)
