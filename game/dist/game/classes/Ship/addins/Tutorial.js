...
class Tutorial {
    ...
    advanceStep() {
        ...
        if (this.currentStep.forceCrewLocation)
            this.ship.crewMembers.forEach((cm) => (cm.location =
        ...
        this.ship.logEntry(`Good luck out there! If you have more questions about the game, check out the How To Play page!`);
        this.cleanUp();
    ...
    cleanUp() {
        for (let id of this.spawnedShips) {
            const toDelete = this.ship.game.ships.find((s) => s.id);
            if (toDelete)
                this.ship.game.removeShip(toDelete);
        }
        for (let id of this.spawnedCaches) {
            const toDelete = this.ship.game.caches.find((k) => k.id);
            if (toDelete)
                this.ship.game.removeCache(toDelete);
        }
    }
}
