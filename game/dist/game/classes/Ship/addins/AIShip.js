...
const itemData = __importStar(
  require('../../presets/items'),
)
class AIShip extends CombatShip_1.CombatShip {
  ...
  tick() {
    ...
    this.weapons.forEach(
      (w) =>
        (w.cooldownRemaining -=
          dist_1.default.getWeaponCooldownReductionPerTick(
            this.level,
          )),
    )
    ...
    if (enemies.length) {
      const randomEnemy =
        dist_1.default.randomFromArray(enemies)
      const distance = dist_1.default.distance(
        randomEnemy.location,
        this.location,
      )
      const randomWeapon = dist_1.default.randomFromArray(
  ...
  addLevelAppropriateItems() {
    dist_1.default.log(
      `Adding items to level ${this.level} ai...`,
    )
    let itemBudget =
      this.level * dist_1.default.aiDifficultyMultiplier
    ...
    while (canAddMoreItems) {
      ...
      const itemToAdd =
        dist_1.default.randomFromArray(validItems)
  ...
  move(toLocation) {
    ...
    if (
      ...
      const unitVectorToTarget =
        dist_1.default.degreesToUnitVector(
          dist_1.default.angleFromAToB(
            this.location,
            this.targetLocation,
          ),
        )
      const thrustMagnitude =
        dist_1.default.getThrustMagnitudeForSingleCrewMember(
          this.level,
    ...
    if (Math.random() < 0.015) {
      ...
      const currentAngle = dist_1.default.angleFromAToB(
        this.location,
        this.targetLocation,
      )
      const possibleAngles = [
        ...
        const diff = dist_1.default.angleDifference(
          a,
          currentAngle,
        )
        return diff > 1 && diff < 179
      })
      const angleToHome = dist_1.default.angleFromAToB(
        this.location,
        this.spawnPoint,
      )
      const chosenAngle =
        Math.random() > 1
          ...
          : possibleAngles.reduce(
              (lowest, a) =>
                dist_1.default.angleDifference(
                  angleToHome,
                  a,
                ) <
                dist_1.default.angleDifference(
      ...
      const unitVector =
        dist_1.default.degreesToUnitVector(chosenAngle)
      this.targetLocation = [
  ...
  die() {
    ...
    const cacheContents = [{ type: `credits`, amount }]
    this.game.addCache({
