...
export function checkAchievements(this: HumanShip, typePrefix?: string) {
  for (let achievement of Object.values(c.achievements).filter(
    ...
    if (condition === true) {
      ...
      if (condition?.membersIn) {
        ...
        for (let con of toCheck) {
          const requiredAmount =
            con.amount === `all` ? this.crewMembers.length : con.amount
          if (this.membersIn(con.roomId).length < requiredAmount) ok = false
        }
      }
      if (ok && condition?.prop) {
        ...
        for (let con of toCheck) {
          if (con.is && !con.secondaryId && this[con.id] !== con.is) ok = false
          else if (
            con.secondaryId &&
            !con.is &&
            ...
            if (con.lte && (this[con.id]?.length || Infinity) > con.amount)
              ok = false
            else if ((this[con.id]?.length || 0) < con.amount) ok = false
          } else if (con.amount) {
      ...
      if (ok && condition?.stat) {
        ...
        for (let con of toCheck) {
          const stat = this.stats.find((s) => s.stat === con.id)
          if (!stat) ok = false
          else if (con.lte && stat.amount > con.amount) ok = false
...
export function addAchievement(
  ...
  if (!Array.isArray(achievementIds)) achievementIds = [achievementIds]
  const newIds = achievementIds.filter((id) => !this.achievements.includes(id))
  ...
  const achievements = Object.values(c.achievements).filter((achievement) =>
    newIds.includes(achievement.id),
  )
...
export function addTagline(
  ...
  if (this.availableTaglines.find((t) => t === tagline)) return
...
export function addHeaderBackground(
  ...
  if (this.availableHeaderBackgrounds.find((b) => b === bg)) return
  this.availableHeaderBackgrounds.push(bg)
  this.toUpdate.availableHeaderBackgrounds = this.availableHeaderBackgrounds

  if (!silent) {
    this.logEntry(
      [
        `Ship header image unlocked for ${reason}:`,
