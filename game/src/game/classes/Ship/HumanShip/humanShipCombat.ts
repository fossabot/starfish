...
export function determineTargetShip(this: HumanShip): CombatShip | null {
  ...
  if (this.membersIn(`weapons`).length === 0) return setTarget(null)
  ...
  const shipTargetCounts = this.membersIn(`weapons`).reduce(
    (totals: { target: CombatShip; total: number }[], cm) => {
      if (cm.attackTargetId === `any`) return totals
      let targetId = cm.attackTargetId
      if (cm.attackTargetId === `closest`) {
        if (!closestShip) closestShip = this.getEnemiesInAttackRange()[0]
        if (closestShip) targetId = closestShip.id
        else return totals
      }
      const existingTotal = totals.find((t) => t.target.id === targetId)
      const skillWeight = cm.strength.level
  ...
  this.idealTargetShip =
    (shipTargetCounts.sort((b: any, a: any) => a.total - b.total)?.[0]
      ?.target as CombatShip | undefined) || null

  const shipTargetCountsWeightedByAttackable = shipTargetCounts.map(
    (totalEntry) => {
  ...
  if (this.combatTactic === `defensive`) {
    if (
      ...
      const attackedByThatTarget = this.visible.attackRemnants.find(
        (ar) => ar.attacker?.id === mostViableManuallyTargetedShip.id,
      )
      if (attackedByThatTarget) {
        return setTarget(mostViableManuallyTargetedShip)
      }
    } else {
      const mostRecentDefense = this.visible.attackRemnants.reduce(
        (
          mostRecent: AttackRemnant | AttackRemnantStub | null,
          ar,
        ): AttackRemnant | AttackRemnantStub | null => {
          // was defense
          if (ar.attacker?.id === this.id || ar.defender?.id !== this.id)
            return mostRecent
      ...
      return setTarget((mostRecentDefense?.attacker as CombatShip) || null)
    }
  }

  // ----- aggressive strategy -----
  else if (
    [`aggressive`, `onlyNonPlayers`, `onlyPlayers`].includes(this.combatTactic)
  ) {
    ...
    if (
      ...
      (this.combatTactic === `onlyNonPlayers` && targetShip?.ai) ||
      (this.combatTactic === `onlyPlayers` && targetShip?.human)
    )
    ...
    const mostRecentCombat = this.visible.attackRemnants.reduce(
      (
        ...
        const targetId = ar.attacker?.id === this.id ? ar.defender : ar.attacker
    ...
    if (!targetShip) {
      targetShip = this.getEnemiesInAttackRange()
        .filter(
          (s) =>
            this.combatTactic === `aggressive` ||
            (this.combatTactic === `onlyNonPlayers` && s?.ai) ||
            (this.combatTactic === `onlyPlayers` && s?.human),
        )
        .reduce((closest: CombatShip | undefined, curr: CombatShip) => {
          if (
            !closest ||
...
export function recalculateCombatTactic(this: HumanShip) {
  const tacticCounts = this.membersIn(`weapons`).reduce((totals: any, cm) => {
    if (!cm.combatTactic || cm.combatTactic === `none`) return totals
    const existingTotal = totals.find((t: any) => t.tactic === cm.combatTactic)
    const toAdd = cm.strength.level
  ...
  const mainTactic =
    (tacticCounts.sort((b: any, a: any) => a.total - b.total)?.[0]
      ?.tactic as CombatTactic) || `pacifist`
...
export function recalculateTargetItemType(this: HumanShip) {
  const memberTargetItemTypeCounts = this.membersIn(`weapons`).reduce(
    (totals: any, cm) => {
  ...
  let mainTargetItemType: ItemType | `any` =
    memberTargetItemTypeCounts.sort((b: any, a: any) => a.total - b.total)?.[0]
      ?.target || `any`
...
// ----- auto attack -----
export function autoAttack(this: HumanShip, predeterminedHitChance?: number) {
  ...
  if (this.targetShip)
    this.availableWeapons()
      .filter(
        (w) =>
          w.effectiveRange >=
          c.distance(this.targetShip!.location, this.location),
      )
      .forEach((w) => {
        res.push(
          this.attack(
