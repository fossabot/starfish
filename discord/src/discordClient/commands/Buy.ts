...
const command: CommandStub = {
  ...
  getDescription(): string {
    return `See cargo options at the current planet, and potentially buy some.`
  },
  ...
  async run(context: InteractionContext) {
    ...
    if (!vendor || (vendor && !vendor.cargo.length)) {
      await context.reply(`${context.ship.planet.name} doesn't sell any cargo.`)
      return
    }
    ...
    const charismaLevel =
      ...
      (context.crewMember.skills.find((s) => s.skill === `charisma`)?.level ||
        1)
    const getPrice = (cargoForSale: PlanetVendorCargoPrice) => {
    ...
    const forSale = vendor.cargo
      ...
      .filter((cargoForSale) =>
        c.canAfford(cargoForSale.price, context.ship!, context.crewMember),
      )
    if (!forSale.length) {
      await context.reply(`You don't have enough to buy anything here.`)
      return
    }
    ...
    waitForButtonChoiceWithCallback({
      ...
      buttons: forSale.map((p) => ({
        label: `${c.r2(
          Math.min(
            c.canAfford(p.price, context.ship!, context.crewMember) || 0,
            maxRemainingSpace,
          ),
          2,
          true,
        )} tons of ${c.capitalize(c.camelCaseToWords(p.id))}:
${c.priceToString(p.price)}/ton`,
      ...
      callback: async (choice) => {
        const forSaleEntry = forSale.find(
          (cargoForSale) => `buy` + cargoForSale.id === choice,
        )
        ...
        if (`data` in res)
          await context.reply(
            `${context.crewMember!.name} bought ${amountToBuy} ton${
              amountToBuy === 1 ? `` : `s`
            } of ${c.capitalize(c.camelCaseToWords(forSaleEntry.id))} from ${
              planet.name
