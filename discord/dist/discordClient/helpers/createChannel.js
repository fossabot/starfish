...
async function resolveOrCreateChannel({ name, topic, permissions = [], guild, }) {
    ...
    // ----- get bot role -----
    const existingRoles = (await guild.roles.fetch()).cache.array();
    const botRole = existingRoles.find((role) => role.name === __1.client.user?.username);
    if (!botRole) {
        return null;
    }
